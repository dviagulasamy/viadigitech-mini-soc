#!/usr/bin/env python3
"""
ViaDigiTech SOC — Rapport mensuel (30 jours)
Exécuté le 1er de chaque mois à 7h via cron.
Génère un rapport HTML sur 30 jours + comparatif M-1 vs M, top IPs, graphique 4 semaines.
"""
...
# CONFIG
# ─────────────────────────────────────────
LOCK_FILE      = "/tmp/report_monthly.lock"
...
OUTPUT_FILE    = "/var/www/html/viadigitech-reports/monthly_report.html"
...
def parse_auth_log_period(since_hours=720):
    """Analyse auth.log sur la période donnée (720h = 30 jours)."""
...
def get_fail2ban_bans_period(since_hours=720):
    """Bans fail2ban sur la période (720h = 30 jours)."""
    bans = Counter()
    since = datetime.now() - timedelta(hours=since_hours)
...
def get_bans_per_week(weeks=4):
    """Retourne le nombre de bans par semaine sur `weeks` semaines depuis audit_actions.csv."""
    week_bans = Counter()
        ...
        for line in f:
            ...
            try:
                ...
                if d >= cutoff and ("BAN_AUTO" in parts[2] or "BAN_OLLAMA" in parts[2]):
                    ...
                    week_label = dt.strftime("S%V\n%d/%m")
                    week_bans[week_label] += 1
            except Exception:
                pass
    return week_bans

def get_top_subnets(audit_days=30, n=10):
    """Retourne les top /24 par nombre de bans depuis audit_actions.csv."""
    subnet_bans = Counter()
    if not os.path.exists(AUDIT_LOG):
        return subnet_bans
    cutoff = (datetime.now() - timedelta(days=audit_days)).isoformat()
    with open(AUDIT_LOG, errors="ignore") as f:
        for line in f:
            if line.startswith("timestamp"):
                continue
            ...
            if len(parts) < 3:
                continue
            if parts[0] >= cutoff and ("BAN_AUTO" in parts[2] or "BAN_OLLAMA" in parts[2]):
                ip = parts[1]
                subnet = ".".join(ip.split(".")[:3]) + ".0/24"
                subnet_bans[subnet] += 1
    return subnet_bans

def get_audit_monthly(days=30):
    """Dernières actions audit sur 30 jours."""
    rows = []
    if not os.path.exists(AUDIT_LOG):
        return rows
...
def get_period_stats(offset_days, span_days=30):
...
def _save(fig, name):
    ...
    path = f"{IMGDIR}/monthly-{name}-{TS}.png"
    fig.savefig(path, dpi=110, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close(fig)
    return f"{IMG_BASE_URL}/monthly-{name}-{TS}.png"

def chart_bans_par_semaine(week_bans):
    """Graphique barres : bans par semaine sur 4 semaines."""
    today = datetime.now().date()
    labels = []
    values = []
    for i in range(3, -1, -1):
        # Lundi de la semaine i semaines en arrière
        monday = today - timedelta(days=today.weekday()) - timedelta(weeks=i)
        label = monday.strftime("S%V\n%d/%m")
        labels.append(label)
        values.append(week_bans.get(label, 0))

    fig, ax = plt.subplots(figsize=(9, 3.5), facecolor=C["surface"])
    ax.set_facecolor(C["surface"])
    max_v = max(values) if values else 1
    colors = [C["red"] if v == max_v and v > 0 else C["accent"] for v in values]
    ...
    for bar, v in zip(bars, values):
        if v > 0:
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + max_v * 0.02,
                    str(v), ha="center", va="bottom", color=C["text"], fontsize=10, fontweight="bold")
    ax.set_title("Bans automatiques — 4 dernières semaines", color=C["text"], fontsize=12, fontweight="bold", pad=10)
    ax.tick_params(colors=C["text"], labelsize=9)
    ax.spines[:].set_color(C["border"])
    ax.set_ylabel("Bans", color=C["muted"], fontsize=9)
    fig.tight_layout()
    return _save(fig, "bans_par_semaine")

def chart_top_ips(failed_ips, n=10):
    """Graphique barres horizontales : top IPs attaquantes sur 30 jours."""
    if not failed_ips:
        return None
    ...
    ax.set_title("Top 10 IPs attaquantes — 30 jours", color=C["text"], fontsize=12, fontweight="bold", pad=10)
...
def build_monthly_ai_analysis(metrics, failed_ips, total_attempts,
                               bans, total_m, bans_m, total_m1, bans_m1,
                               top_subnets):
    now = datetime.now()
    mois_nom = now.strftime("%B %Y")

    delta_att  = total_m - total_m1
    delta_bans = bans_m  - bans_m1
    ...
    top_ips_str = str(failed_ips.most_common(5))
    top_subnets_str = str(top_subnets.most_common(5))

    prompt = f"""IMPORTANT : réponds UNIQUEMENT en français, sans mélanger d'autres langues.

Tu es l'analyste SOC du serveur VPS ViaDigiTech. Voici le bilan mensuel ({mois_nom}) :

MÉTRIQUES ACTUELLES :
- CPU : {metrics['cpu_percent']}% | RAM : {metrics['mem_percent']}% | Disque : {metrics['disk_percent']}%

SÉCURITÉ SSH — 30 DERNIERS JOURS :
- Tentatives totales : {total_attempts}
- IPs uniques : {len(failed_ips)}
- Top 5 IPs attaquantes : {top_ips_str}
- Top 5 sous-réseaux /24 : {top_subnets_str}
- Bans fail2ban total : {sum(bans.values())}

COMPARATIF MOIS PRÉCÉDENT :
- Tentatives SSH : {total_m} ce mois vs {total_m1} le mois précédent ({delta_att_txt})
- Bans fail2ban : {bans_m} ce mois vs {bans_m1} le mois précédent ({delta_bans_txt})

Rédige un bilan mensuel en 4 points numérotés, en français :
1. BILAN GLOBAL : résume le mois en 2 phrases (menace globale, tendance)
2. POINTS CHAUDS : les 2-3 éléments les plus préoccupants ce mois
3. ÉVOLUTION vs M-1 : interprète le comparatif (aggravation / amélioration / stable)
4. RECOMMANDATIONS : 2-3 actions concrètes à envisager pour le mois prochain

Maximum 250 mots. Sois factuel et opérationnel. Signe "— SOC IA ViaDigiTech"."""

    print("  → Synthèse mensuelle IA...")
...
def build_html(metrics, failed_ips, total_attempts, accepted,
               bans, week_bans, top_subnets, ai_analysis, charts,
               total_m, bans_m, total_m1, bans_m1):
    now = datetime.now()
    hostname = os.uname().nodename
    mois_nom = now.strftime("%B %Y")
    mois_prev = (now.replace(day=1) - timedelta(days=1)).strftime("%B %Y")
    ...
    # Top /24
    top_subnet_rows = "".join(
        f'<tr><td style="color:#64748b;width:28px">{i}</td>'
        f'<td style="font-family:monospace;color:#a78bfa">{subnet}</td>'
        f'<td style="text-align:right;padding-right:16px;font-weight:700;color:#f97316">{cnt}</td></tr>'
        for i, (subnet, cnt) in enumerate(top_subnets.most_common(10), 1)
    )

    # Audit récent
    audit_rows = ""
    for parts in get_audit_monthly(30)[:30]:
...
<head>
<meta charset="UTF-8">
<title>SOC — Rapport Mensuel {mois_nom}</title>
<style>
...
<div class="card" style="border-top:3px solid #a78bfa">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px">
    <div>
      <div style="font-size:22px;font-weight:700;color:#fff">&#128737; AI SecOps — Rapport Mensuel</div>
      <div style="font-size:13px;color:#a0aec0;margin-top:4px">{hostname} &nbsp;&#183;&nbsp; {mois_nom} &nbsp;&#183;&nbsp; {now.strftime('%d/%m/%Y %H:%M')}</div>
    </div>
  ...
  <div style="margin-top:16px;display:flex;flex-wrap:wrap">
    ...
    <span class="pill"><span class="dot dot-red"></span>{total_attempts} tentatives SSH (30j)</span>
    <span class="pill"><span class="dot dot-yellow"></span>{sum(bans.values())} bans (30j)</span>
  </div>
</div>

<!-- SYNTHESE IA MENSUELLE -->
<div class="card" style="border-top:3px solid #818cf8">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
    <span style="font-size:22px">&#129302;</span>
    <div>
      <div style="font-weight:700;color:#ffffff;font-size:15px">Bilan du mois de {mois_nom}</div>
      <div style="font-size:11px;color:#a5b4fc;font-weight:600;letter-spacing:.5px">SYNTHESE MENSUELLE &middot; {OLLAMA_MODEL.upper()}</div>
    </div>
  </div>
...
<!-- COMPARATIF M vs M-1 -->
<div class="card">
  <div class="title">&#128202; Comparatif — {mois_nom} vs {mois_prev}</div>
  <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-bottom:16px">
    <div class="metric">
      <div class="mval" style="color:#ef4444">{total_m}</div>
      <div class="mlabel">Tentatives SSH (M)</div>
      <div class="msub">{delta_badge(delta_att)} vs M-1 ({total_m1})</div>
    </div>
    <div class="metric">
      <div class="mval" style="color:#eab308">{bans_m}</div>
      <div class="mlabel">Bans fail2ban (M)</div>
      <div class="msub">{delta_badge(delta_bans)} vs M-1 ({bans_m1})</div>
    </div>
  </div>
</div>
...
<!-- METRIQUES SYSTEME -->
<div class="card">
  <div class="title">&#128421; Métriques Système (actuelles)</div>
...
<!-- GRAPHIQUE BANS 4 SEMAINES -->
<div class="card">
  <div class="title">&#128200; Graphique bans automatiques — 4 semaines</div>
  {img(charts.get('bans_par_semaine'), 'Bans 4 semaines', 0)}
</div>

<!-- TOP 10 IPs + TOP /24 -->
<div class="card">
  <div class="title">&#128272; Sécurité SSH — 30 jours</div>
  ...
  <div style="margin-top:18px">
    <div style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Top 10 IPs attaquantes (30 jours)</div>
  ...
  <div style="margin-top:18px">
    <div style="font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Top 10 sous-réseaux /24 bannés (30 jours)</div>
    <table><thead><tr><th>#</th><th>Sous-réseau</th><th style="text-align:right;padding-right:16px">Bans</th></tr></thead>
    <tbody>{top_subnet_rows}</tbody></table>
  </div>
</div>

<!-- AUDIT ACTIONS -->
{"" if not audit_rows else '<div class="card"><div class="title">&#128269; Audit actions SOC — 30 jours</div><table><thead><tr><th>Horodatage</th><th>IP</th><th>Action</th><th>Score</th></tr></thead><tbody>' + audit_rows + '</tbody></table></div>'}
...
<div style="text-align:center;margin-top:8px;padding:16px;border-top:1px solid #1e2035">
  <a href="{DASHBOARD_URL}" style="display:inline-block;background:linear-gradient(135deg,#312e81,#1e1b4b);border:1px solid #4338ca;color:#a5b4fc;text-decoration:none;padding:9px 22px;border-radius:8px;font-size:13px;font-weight:600;margin-bottom:12px">
    &#128421; Ouvrir le Dashboard SOC
  </a>
  <div style="font-size:11px;color:#334155;margin-top:8px">
    <span style="color:#a0aec0">AI SecOps Monthly &middot; {hostname} &middot; {mois_nom} &middot; {now.strftime('%d/%m/%Y %H:%M')} &middot; {OLLAMA_MODEL}</span>
  </div>
</div>
...
def main():
    ...
    try:
        ...
        print(f"[{datetime.now():%H:%M:%S}] Rapport mensuel déjà en cours — abandon.")
        return

    now = datetime.now()
    mois_nom = now.strftime("%B %Y")
    print(f"[{now:%H:%M:%S}] Rapport mensuel — {mois_nom}")

    print(f"[{now:%H:%M:%S}] Collecte métriques système...")
    metrics = get_system_metrics()

    print(f"[{now:%H:%M:%S}] Analyse logs SSH (30 jours)...")
    failed_ips, failed_users, accepted, total = parse_auth_log_period(720)
    bans = get_fail2ban_bans_period(720)
    week_bans = get_bans_per_week(4)

    print(f"[{now:%H:%M:%S}] Calcul top /24...")
    top_subnets = get_top_subnets(30)

    print(f"[{now:%H:%M:%S}] Comparatif M vs M-1...")
    total_m,  bans_m  = get_period_stats(0,  30)
    total_m1, bans_m1 = get_period_stats(30, 30)

    print(f"[{now:%H:%M:%S}] Génération des graphiques...")
    charts = {
        "bans_par_semaine": chart_bans_par_semaine(week_bans),
        "top_ips":          chart_top_ips(failed_ips),
    }
    print(f"  Graphiques : {[k for k, v in charts.items() if v]}")

    print(f"[{now:%H:%M:%S}] Synthèse mensuelle IA Ollama (peut prendre 1-3 min)...")
    ai_analysis = build_monthly_ai_analysis(
        metrics, failed_ips, total,
        bans, total_m, bans_m, total_m1, bans_m1,
        top_subnets
    )
    ...
    html = build_html(
        metrics, failed_ips, total, accepted,
        bans, week_bans, top_subnets, ai_analysis, charts,
        total_m, bans_m, total_m1, bans_m1
    )
    ...
    # Sujet mail avec niveau de menace
    ban_count = sum(bans.values())
    if ban_count > 400 or total > 8000:
        icon, label = "&#128308;", "CRITIQUE"
    elif ban_count > 200 or total > 4000:
        icon, label = "&#128992;", "ELEVEE"
    ...
    mois_annee = now.strftime("%B %Y")
    subject = f"[SOC] Rapport mensuel ViaDigiTech — {mois_annee} · Menace {label} · {ban_count} bans · {total} tentatives SSH"
    print(f"[{now:%H:%M:%S}] Envoi mail à {MAIL_TO}...")
    send_mail(html, subject)
    print(f"[{now:%H:%M:%S}] Rapport mensuel envoyé.")
